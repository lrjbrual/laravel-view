...
class Reimbursement extends Model
{
    ...
    public function isExist($data=array()){
    	...
    	$query = $db_query->table('reimbursements')->select('*');
    	$query->where( 'reimbursement_id', $data['reimbursement_id'] );
    ...
    public function insertData($data = array()){
        ...
        if($data!=null AND count($data)>0){
            $db_query->table('reimbursements')->insert($data);
            return true;
        }else{
            return false;
        }
    ...
    public function getRecords($fields = array('*'),$cond=array(),$order=array(),$checkifempty=false){
        $q = DB::connection('mysql2')->table('reimbursements');
        $q = $q->select($fields);
